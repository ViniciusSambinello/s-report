...
- [x] 14.1 Confirm no `.java` file under `src/main/java` contains a comment and that all comments live in `.yml` files. (grep-verified across all three modules, zero matches; Checkstyle's RegexpSinglelineJava enforces this on every build)
- [x] 14.2 Confirm every field and local is `final`, every data type is a record, and no returned collection is mutable. (grep-verified; the only non-final fields are the two documented holder classes — `ReportMenuHolder`'s page/reports/inventory and `SReportsPaperPlugin`'s onEnable-assigned bootstrap fields, which cannot be constructor-final under the JavaPlugin lifecycle)
- [x] 14.3 Confirm every identifier, log line, and player-facing default string is in English. (grep-verified: zero non-ASCII characters anywhere under src/main/java)
- [x] 14.4 Build both shaded jars and confirm relocation applied and no shaded class leaks into the default package space. (verified via jar inspection: 0 unrelocated com.zaxxer/com.mysql/org.yaml classes; also excluded mysql-connector-j's unused protobuf-java (X DevAPI, not used — we're classic-JDBC-only via HikariCP) from both shaded jars, shrinking them from ~5.1MB to ~3.3MB. Jar inspection alone missed a real runtime defect the next step caught — see below.)
- [ ] 14.5 Manually verify the single-server path: report, notification, menu, left-click teleport, right-click dismiss, `/togglereport`. (requires a live Paper+MySQL deployment — see note below. Infrastructure now live-verified: booted the shaded jars against a real Velocity proxy + two Paper 26.2 backends + MySQL; this caught and fixed a real bug — the shaded MySQL driver failed to register at runtime (HikariCP fell back to ServiceLoader-based DriverManager.getDriver(), which Shadow's relocation doesn't handle by default). Fixed via mergeServiceFiles()+duplicatesStrategy=INCLUDE plus an explicit driverClassName from a compile-time class literal. All three processes now enable cleanly with working HikariCP pools and the schema created in a real database. In-game click-through (chat, menu, teleport) still needs a real Minecraft client, which this environment doesn't have.)
- [ ] 14.6 Manually verify the cross-server path: report on server A, browse and left-click from server B, arriving next to the target on server A. (requires a live Paper+Velocity+MySQL deployment — see note below)
- [ ] 14.7 Manually verify the empty-server path: file a report while a backend is empty, join it, and confirm reconciliation surfaces the report. (requires a live multi-server deployment — see note below)
- [ ] 14.8 Manually verify the degraded paths: proxy unavailable, database unavailable, and target disconnecting mid-transfer. (requires a live multi-server deployment — see note below)

**Note on 14.5-14.8**: these require running real Paper + Velocity server processes with connected game clients on a live network, which this environment cannot provide (no Minecraft client/game connection available here). Everything checkable without that — full-suite compilation, Checkstyle, 56 automated tests including a real Testcontainers MySQL instance, and static source/config audits — has been verified. These four remain for the operator to run against an actual deployment, following the Migration Plan in `design.md`.
