...
- [x] 14.4 Build both shaded jars and confirm relocation applied and no shaded class leaks into the default package space. (verified via jar inspection: 0 unrelocated com.zaxxer/com.mysql/org.yaml classes; also excluded mysql-connector-j's unused protobuf-java (X DevAPI, not used — we're classic-JDBC-only via HikariCP) from both shaded jars, shrinking them from ~5.1MB to ~3.3MB)
- [ ] 14.5 Manually verify the single-server path: report, notification, menu, left-click teleport, right-click dismiss, `/togglereport`. (requires a live Paper+MySQL deployment — see note below)
